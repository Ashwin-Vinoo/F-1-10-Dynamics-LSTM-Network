...
class DynamicsModelRNN(nn.Module):
    ...
    def norm_train_network(self, train_dataset, epochs, test):
        ...
        for epoch in range(epochs * sequence_length):
            ...
            self.norm_evaluate_network( test, epoch)
                    # Converting the network back into training mode
            self.train()

    # This is a function to evaluate the neural network using normalized data and labels
    def norm_evaluate_network(self, test_dataset, i):
        # Making the network to be in evaluation mode so that losses won't be accumulated, thereby saving memory
        self.eval()
        ...
        print('Epoch %4d  Average Loss Over the Test Dataset is: %.6f' % (i, total_sequence_loss/test_dataset.__len__()))
...
if __name__ == "__main__":
    ...
    net.norm_train_network(training_dataset, training_epochs, testing_dataset)
