...
dynamics_data_file = "dynamics_data_dnn.csv"
...
trained_network_state_dict_file = 'trained_network_dict_dnn.pth'
# The name of the file from which we can load the dynamics data parameters
dynamics_data_parameters_file = 'dynamics_data_parameters_dnn.pkl'
...
class DynamicsModelDNN(nn.Module):
    ...
    def norm_train_network(self, train_data_loader, epochs, test_data_loader):
        ...
        for epoch in range(epochs):
            ...
            self.norm_evaluate_network(test_data_loader, epoch)

    # This is a function to evaluate the neural network using normalized data and labels
    def norm_evaluate_network(self, test_data_loader, i):
        # The running loss is initialized to zero
        running_loss = 0.0
        # Making the network to be in evaluation mode so that losses won't be accumulated, thereby saving memory
        self.eval()
        ...
        print('Epoch %3d    Average Loss Over the Training Dataset is: %.8f' % (i, running_loss/len(test_data_loader)))
...
if __name__ == "__main__":
    ...
    net.norm_train_network(train_loader, training_epochs, test_loader)
